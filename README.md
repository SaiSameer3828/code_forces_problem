...
1. Set your Space handle in `app.js`:
	```js
	// app.js
	const SPACE = "Sai-Sameer-09/CF_Problem_Recommender";
...
- **Space target:**  
  ...
  ```js
  const SPACE = "Sai-Sameer/CF_Problem_Recommender";
  ```
  Full URL form (works the same):
  ```js
  const SPACE = "https://sai-Sameer-problem-recommender.hf.space/";
  ```
